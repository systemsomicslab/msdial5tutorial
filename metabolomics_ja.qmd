...
## 3. ユーザーが定義したMS/MSデータベース（MSP形式）を使用したLC/MSあるいはLC/MS/MS(data dependent MS/MS)プロジェクト

ここでは、ユーザー定義のMSPライブラリ（MassBank、GNPS、およびRespectの統合ライブラリ）と組み合わせたdata dependent MS/MSのプロジェクトについて説明します。

### 3-1 入力データ

このセクションでは合計6つのwiffファイルを使用します。MSPファイルはこのデモの同じフォルダに含まれています。 
実験プロトコルについては、過去の研究を参照してください: <http://pubs.acs.org/doi/abs/10.1021/acs.jafc.5b04890>.

デモファイル一式: <https://zenodo.org/records/10952446>

lcms_dda_metabolomics.zipをダウンロードしてください。

![Fig1](images/lcms-dda/Fig1.png)

![Fig2](images/lcms-dda/Fig2.png)
...
-   **溶液A:**  0.1%酢酸水溶液
-   **溶液B:**  0.1%酢酸アセトニトリル溶液
-   **質量分析:** data dependentのネガチィブイオンモード
...
1.  MS-DIALを起動し、New Projectを選択します。「Project title」を入力し、「project file path」にwiffファイルを含むフォルダを指定し、Nextをクリックします。
![Fig3](images/lcms-dda/Fig3.png)

2.  「Browse」ボタンをクリックし、ファイル形式からWIFF fileを選択し、インポートするwiffファイルを一括選択します。
![Fig4](images/lcms-dda/Fig4.png)
![Fig5](images/lcms-dda/Fig5.png)

### 3-3 パラメーターの設定

1.  データがインポートされたことを確認し、必要に応じてファイルの情報を修正し、Nextをクリックします。
例：Class IDを1,2,3...からA,B,C...に修正します。エクセルなどのテキストデータを貼り付けることも可能です。
![Fig6](images/lcms-dda/Fig6.png)

2.  “Measurement parameters” セクション：規定値のままNextをクリック
![Fig7](images/lcms-dda/Fig7.png)

3.  “Data collection”セクション：Multithreading（使用するスレッド数）を使用するマシンのスペックに合わせて変更し、Nextをクリック
![Fig8](images/lcms-dda/Fig8.png)

4.  “Peak detection”セクション、“Spectrum deconvolution”セクション：規定値のままNextをクリック
![Fig9](images/lcms-dda/Fig9.png)
![Fig10](images/lcms-dda/Fig10.png)

5.  “Identification”セクション： (1)Database settingのプラスマークをクリック　(2)Database typeはMspを選択　(3)Brouseボタンからmspファイルを選択
![Fig11](images/lcms-dda/Fig11.png)

6.  “Adduct ion”セクション、“Alignment parameters”セクション、“lsotope tracking”セクション： 規定値のままNext
![Fig12](images/lcms-dda/Fig12.png)
![Fig13](images/lcms-dda/Fig13.png)

7.  最後にRunをクリックをクリックして実行します。
![Fig14](images/lcms-dda/Fig14.png)

### 3-4 アノテーションの結果
![Fig15](images/lcms-dda/Fig15.png)
これはMS-DIALのメインビューアです。(1)File navigatorでファイル名をダブルクリックすると、中央のウィンドウ（Peak spot viewer）に検出されたピーク情報が表示されます。次にPeak spot viewerの隣のAlignment spot viewerをクリックしてアライメント結果を確認します。青いスポットはサンプル中の含有量の少ないピーク、赤いスポットは含有量の多いピーク、緑のスポットは含有量が中程度のピークを示します。(2)中央のウィンドウのスポットをクリックすると、そのピークスポットに対応する情報が表示されます。(3) 中央のウィンドウの左側のウィンドウに、注目したピークのMS1スペクトルが表示されます。(4) 上部のウィンドウに、注目したピークのイオンクロマトグラムが表示されます。(5) 右上のウィンドウに、その他のピーク情報が表示されます。

<!--### The video tutorial for the MS-DIAL operation-->
ここまでのチュートリアルの操作については以下のビデオでも確認することができます。
