...
MS-DIAL の最新バージョンでは、新しいグラフィカルインターフェイスによってユーザーエクスペリエンスが大幅に改善されており、異なるベンダーフォーマットから生の MS データを共通の ABF フォーマットに変換する必要がなくなり、直接インポートしていただけるようになりました。(サポートされているファイルフォーマットや分析技術のまとめは Figure 1 になります。)
